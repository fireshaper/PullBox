...
class MetadataProvider(ABC):
    ...
    async def resolve_metron_id(self, comicvine_id: str | None) -> str | None:
        """Metron's id for a ComicVine-sourced record, when it can be found.

        Base implementation: nothing to resolve. ``CompositeProvider`` overrides it.
        """
        return None
...
class ComicVineProvider:
    ...
    async def get_issue(self, comicvine_id: str) -> dict:
        ...
        return {
            **record,
            "metron_id": None,
            "series": {**series, "metron_id": None} if series else None,
            "story_arcs": arcs,
        }
...
class CompositeProvider(MetadataProvider):
    ...
    async def resolve_metron_id(self, comicvine_id: str | None) -> str | None:
        """Find the Metron series id matching ``comicvine_id``, or None.

        Lets a series that was added from ComicVine graduate to Metron: once the
        caller persists the returned id, every later id-based call routes to the
        primary source instead of being pinned to ComicVine forever. Best-effort —
        a provider error means "couldn't resolve", never a failed sync.
        """
        if self._metron is None or self._primary != "metron" or not comicvine_id:
            return None
        try:
            record = await self._metron.find_series_by_cv_id(comicvine_id)
        except _PROVIDER_ERRORS as exc:
            logger.warning("resolve_metron_id for cv %s failed: %s", comicvine_id, exc)
            return None
        return record.get("metron_id") if record else None
