...
class MetronClient:
    ...
    async def find_series_by_cv_id(self, comicvine_id: str) -> dict | None:
        """Look up a series by its ComicVine id, or None if Metron doesn't have it.

        Metron's ``/series/`` list endpoint accepts a ``cv_id`` filter, which is the
        only bridge between a ComicVine-sourced series row and its Metron record —
        without it a series added before Metron was configured stays on ComicVine
        forever. The returned row is a sparse list row (``_map_series`` fills what
        it can); callers that want full metadata follow up with ``get_volume``.
        """
        if not comicvine_id:
            return None
        data = await self._get("/series/", cv_id=comicvine_id)
        results = data.get("results") or []
        if not results:
            return None
        return self._map_series(results[0])
    ...
    async def get_issue(self, metron_id: str) -> dict:
        ...
        return {
            ...
            "series": self._embedded_series(data),
            "story_arcs": arcs,
        }

    def _embedded_series(self, item: dict) -> dict | None:
        """The series block on an issue detail, in the normalized series shape.

        Lets an issue reached by id alone (a story-arc member PullBox doesn't own)
        create its local Series row without a follow-up ``/series/`` call. The
        embedded object is sparse — no publisher on some responses — so callers
        treat the missing fields as "not known yet", not as "empty".
        """
        series = item.get("series")
        if not isinstance(series, dict) or not series.get("id"):
            return None
        return {
            "metron_id": str(series["id"]),
            "comicvine_id": str(series["cv_id"]) if series.get("cv_id") else None,
            "title": series.get("name") or "",
            "publisher": self._publisher_name(item) or self._publisher_name(series),
