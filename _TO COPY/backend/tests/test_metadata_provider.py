...
# ── resolve_metron_id (ComicVine → Metron graduation) ─────────────────────────


async def test_resolve_metron_id_returns_the_matching_series_id():
    metron = _fake_source("metron")
    metron.find_series_by_cv_id.return_value = {"metron_id": "m1", "comicvine_id": "c1"}

    provider = CompositeProvider(metron=metron, comicvine=_fake_source("comicvine"))
    assert await provider.resolve_metron_id("c1") == "m1"
    metron.find_series_by_cv_id.assert_awaited_once_with("c1")


async def test_resolve_metron_id_is_none_when_metron_does_not_know_it():
    metron = _fake_source("metron")
    metron.find_series_by_cv_id.return_value = None

    provider = CompositeProvider(metron=metron, comicvine=_fake_source("comicvine"))
    assert await provider.resolve_metron_id("c1") is None


async def test_resolve_metron_id_swallows_provider_errors():
    metron = _fake_source("metron")
    metron.find_series_by_cv_id.side_effect = MetronError("down")

    provider = CompositeProvider(metron=metron, comicvine=_fake_source("comicvine"))
    assert await provider.resolve_metron_id("c1") is None


async def test_resolve_metron_id_skipped_when_comicvine_is_primary():
    metron = _fake_source("metron")
    provider = CompositeProvider(
        metron=metron, comicvine=_fake_source("comicvine"), primary="comicvine"
    )
    assert await provider.resolve_metron_id("c1") is None
    metron.find_series_by_cv_id.assert_not_awaited()
