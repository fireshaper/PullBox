...
# ── delete_empty_folder cleanup ───────────────────────────────────────────────


def _job_layout(tmp_path: Path, name: str = "Batman 012") -> tuple[Path, Path, Path]:
    """Build a client-style completed layout: <complete>/<job folder>/<file>.cbz."""
    complete = tmp_path / "complete"
    job = complete / name
    job.mkdir(parents=True)
    comic = job / "Batman 012.cbz"
    comic.write_bytes(b"data")
    return complete, job, comic


def test_move_removes_empty_job_folder_when_enabled(tmp_path):
    complete, job, comic = _job_layout(tmp_path)
    library = tmp_path / "library"

    result = apply_post_processing(
        ISSUE, SERIES, str(comic), _cfg(delete_empty_folder=True), str(library)
    )

    assert result is not None and Path(result).exists()
    assert not job.exists()  # job folder cleaned up
    assert complete.exists()  # shared completed root untouched


def test_move_keeps_job_folder_when_disabled(tmp_path):
    _complete, job, comic = _job_layout(tmp_path)
    library = tmp_path / "library"

    apply_post_processing(
        ISSUE, SERIES, str(comic), _cfg(delete_empty_folder=False), str(library)
    )

    assert job.exists()  # opt-in only


def test_move_keeps_job_folder_that_still_has_files(tmp_path):
    _complete, job, comic = _job_layout(tmp_path)
    (job / "readme.nfo").write_text("leftover")
    library = tmp_path / "library"
    ...
    assert job.exists()  # not empty → left alone
    assert (job / "readme.nfo").exists()


def test_copy_never_removes_job_folder(tmp_path):
    _complete, job, comic = _job_layout(tmp_path)
    library = tmp_path / "library"
    ...
    apply_post_processing(
        ...
        SERIES,
        str(comic),
        _cfg(operation="copy", delete_empty_folder=True),
        str(library),
    )

    assert job.exists() and comic.exists()  # original stays put


def test_move_collapses_nested_empty_dirs(tmp_path):
    """A directory report lets us collapse extraction subfolders up to the job folder."""
    complete, job, _comic = _job_layout(tmp_path)
    nested = job / "extracted"
    nested.mkdir()
    comic = nested / "Batman 012.cbz"
    comic.write_bytes(b"data")
    (job / "Batman 012.cbz").unlink()
    library = tmp_path / "library"

    # completed_path is the job folder itself — the client sanctioned that folder.
    apply_post_processing(
        ISSUE, SERIES, str(job), _cfg(delete_empty_folder=True), str(library)
    )

    assert not nested.exists()
    assert not job.exists()  # collapsed up to the job folder
    assert complete.exists()  # shared root untouched


def test_move_stops_at_reported_path_for_file_reports(tmp_path):
    """A file report only sanctions its own folder — never the folder above it."""
    complete, job, _comic = _job_layout(tmp_path)
    nested = job / "extracted"
    nested.mkdir()
    comic = nested / "Batman 012.cbz"
    ...
    assert not nested.exists()  # the folder that held the comic
    assert job.exists()  # we do not guess above what the client reported
    assert complete.exists()


def test_move_keeps_completed_root_holding_other_jobs(tmp_path):
    """The client's shared completed folder survives while other jobs sit in it."""
    complete, _job, comic = _job_layout(tmp_path)
    (complete / "Another Job 5").mkdir()
    library = tmp_path / "library"
    ...
    assert complete.exists()  # non-empty → never removed
    assert (complete / "Another Job 5").exists()


def test_move_does_not_delete_library_root(tmp_path):
    """Never remove the destination root even if the download landed inside it."""
    library = tmp_path / "library"
    job = library / "incoming"
    job.mkdir(parents=True)
    comic = job / "Batman 012.cbz"
    comic.write_bytes(b"data")
